...
from PySide6.QtWidgets import QFileDialog, QFrame, QHBoxLayout, QLabel, QListWidget, QListWidgetItem, QMainWindow, QSplitter, QStackedWidget, QTableWidget, QTableWidgetItem, QToolBar, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, evidence_path=None, logger=None):
        ...
        hostname = QLabel(f"Hostname: {machine_info.get('hostname', 'N/A')}")
        username = QLabel(f"Username: {machine_info.get('username', 'N/A')}")
        os_release = QLabel(f"OS: {machine_info.get('os_version', 'N/A')}")
        info_layout.addWidget(hostname)
        info_layout.addWidget(username)
        info_layout.addWidget(os_release)
